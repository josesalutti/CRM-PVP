...
/** Catalogue key stems — the copy itself lives in the JoinPage namespace. */
const ROLE_KEY: Record<PeekOk['role'], 'roleAdmin' | 'roleAgent' | 'roleViewer'> = {
  admin: 'roleAdmin',
  agent: 'roleAgent',
  viewer: 'roleViewer',
};
...
const FAIL_KEY: Record<
  PeekFail['reason'],
  'failNotFound' | 'failUsed' | 'failExpired' | 'failServer'
> = {
  not_found: 'failNotFound',
  used: 'failUsed',
  expired: 'failExpired',
  server_error: 'failServer',
};
...
export default function JoinPage() {
  ...
  const handleAccept = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 409) {
          setConflictMessage(
            payload.error || t('conflictFallback'),
          );
        } else {
  ...
  const inviteHeader = (
    <CardHeader className="items-center text-center">
      ...
      <CardTitle className="text-xl text-foreground">
        {t.rich('invitedTo', {
          name: () => <span className="text-primary">{peek.account_name}</span>,
        })}
      </CardTitle>
      <CardDescription className="text-muted-foreground">
        {t.rich('joinAs', {
