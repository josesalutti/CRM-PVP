...
/**
 * Canvas label for an outgoing slot. `next` / `true` / `false` are
 * generated by the edge builder in English, so they go through the
 * catalogue; every other slot label is the title the user typed on a
 * button or list row and is shown verbatim.
 */
function slotLabel(
  slot: { id: string; label: string },
  t: ReturnType<typeof useTranslations>,
): string {
  if (slot.id === "next") return t("slotNext");
  if (slot.id === "true") return t("slotTrue");
  if (slot.id === "false") return t("slotFalse");
  return slot.label;
}
      ...
      {isMultiSlot && (
        <div className="border-border mt-2.5 flex flex-col gap-1 border-t pt-2.5">
          {slots.map((slot) => (
            ...
            >
              <span className="truncate" title={slotLabel(slot, t)}>
                {slotLabel(slot, t)}
              </span>
              <Handle
                type="source"
